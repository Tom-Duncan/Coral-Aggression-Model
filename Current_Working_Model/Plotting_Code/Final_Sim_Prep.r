# =============================================================================
#  FINAL SIM PREP - coverage check before the natural-vs-aquarium runs
# -----------------------------------------------------------------------------
...
.cwm <- .ancestorWith("Current_Working_Model")
MODEL_DIR    <- if (!is.na(.cwm)) file.path(.cwm, "Current_Working_Model") else
  "C:/Users/dell/Documents/Research Project 2/Modelling/Current_Working_Model"
REAL_INT_MAT <- file.path(MODEL_DIR, "Plotting_Code", "Real_Int_Mat.r")
...
#  Each name maps to a matrix object defined in Real_Int_Mat.r (10 species each,
...
runFinalCommunities <- function(communities = c("logan10", "heron"),
  ...
  if (file.exists(MIA_MATRIX)) source(MIA_MATRIX)        # Mia (Webden) matrices — unpublished, omitted from this release (see README)
